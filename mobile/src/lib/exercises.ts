// Ported from the web app. NOTE: buildLearnSteps below is CUSTOMIZED for the
// mobile learn flow (per-word: flashcard -> drills, including "Điền từ" and
// "Nghe và viết lại"). The rest stays in sync with web src/lib/exercises.ts.
...
/**
 * Learn-new session — mirrors MochiDemy's real "Học từ mới" flow exactly
 * (verified by playing learn.mochidemy.com). PER WORD, in order:
 ...
 * then move on to the next word. There are NO multiple-choice drills while
 * learning new words — those only appear during review (Ôn tập).
 * listen_write is skipped for the rare word with no audio.
 */
